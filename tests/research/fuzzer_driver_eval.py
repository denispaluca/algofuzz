...
def evaluate_contract(contract, timeout_seconds, reps):
    ...
    for chosen_fuzzer in fuzzers:
        for j in range(3):
            driver = Driver(j)
            info = f"{chosen_fuzzer.__name__}_{driver}"
            config_info[info] = []
    ...
    file = open("benchmarks/metrics/averages.json", "w")
    file.write(json.dumps(averages, indent=2))
    file.close()

    file = open("benchmarks/metrics/max_for_metric.json", "w")
    file.write(json.dumps(max_for_metric, indent=2))
    file.close()

    file = open("benchmarks/metrics/min_for_metric.json", "w")
